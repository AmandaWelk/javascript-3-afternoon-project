...
//Code Here
let totalPopulation = populations.reduce(function(total, pops) {
  return total + pops;
});
...
let orderTotals = orders.map(function(totals) {
  return totals.price * (1 + totals.tax);
});
...
let bobsTotal = purchases.reduce(function(acc, name){
  if (name.owner === "Bob"){
    acc += name.price
  } return acc
},0)
